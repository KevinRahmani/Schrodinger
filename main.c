...
int remplirTabV2(char* fic, float** graphRK4, int n)
{
    FILE* tabfile = fopen(fic,"w+");
    for (int i=0; i<=n; i++)
    {
        fprintf(tabfile, "x=%f , yn=%f, zn=%f \n", graphRK4[i][0], graphRK4[i][1], graphRK4[i][2]);
    }
    fclose(tabfile);
...
float Cn(int i, float h, float yn, float zn, float tn, float B1, float B2){
  return h * f(i, yn + B1/2, zn + B2/2, tn + h/2);
}

float Dn(int i, float h, float yn, float zn, float tn, float C1, float C2){
...
{
  int n; 
  int correct= 1;    int choixMethode;
  ...
  while (choixMethode !=1 && choixMethode !=2 && choixMethode !=3)
    {
      printf("Il y à une erreur dans la saisie de la méthode, veuiller entrer une réponse correcte.\n");
      scanf("%d", &choixMethode);

    }
  
  printf("Bonjour, Veuillez rentrer le nombre de points que vous souhaitez calculer :");
  scanf("%d",&n);
  ...
  switch(choixMethode)
    {
      ...
      case 3:
      printf("on résout avec euler attendez !!!");
      break;
    ...
    remplirTabV2("truc.txt",tableau,n);
     system("cat truc.txt");
